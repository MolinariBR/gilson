import './Sidebar.css'
import { assets } from '../../assets/assets'
import { NavLink } from 'react-router-dom'
import { getAdminTranslation } from '../../constants/adminTranslations'

const Sidebar = () => {
  return (
    <div className='sidebar'>
      <div className="sidebar-options">
        <NavLink to='add' className="sidebar-option">
          <img src={assets.add_icon} alt="" />
          <p>{getAdminTranslation('navigation.addItems', 'Add Items')}</p>
        </NavLink>
        <NavLink to='list' className="sidebar-option">
          <img src={assets.order_icon} alt="" />
          <p>{getAdminTranslation('navigation.listItems', 'List Items')}</p>
        </NavLink>
        <NavLink to='orders' className="sidebar-option">
          <img src={assets.order_icon} alt="" />
          <p>{getAdminTranslation('navigation.orders', 'Orders')}</p>
        </NavLink>
        <NavLink to='zones' className="sidebar-option">
          <img src={assets.parcel_icon} alt="" />
          <p>{getAdminTranslation('navigation.deliveryZones', 'Delivery Zones')}</p>
        </NavLink>
        <NavLink to='categories' className="sidebar-option">
          <img src={assets.add_icon} alt="" />
          <p>{getAdminTranslation('navigation.categories', 'Categories')}</p>
        </NavLink>
      </div>
    </div>
  )
}

export default Sidebar
