...
export const getPhoneValidationError = (phone) => {
  ...
  return 'Formato de telefone inválido. Use (11) 99999-9999 ou (11) 9999-9999';
};
...
/**
 * Mongoose validator function for phone field
 * @param {string} phone - Phone number to validate
 * @returns {boolean} True if valid, false otherwise
 */
export const mongoosePhoneValidator = (phone) => {
  return isValidPhone(phone);
};

/**
 * Express middleware validator for phone in request body
 * @param {string} fieldName - Name of the phone field in request body
 * @returns {Function} Express middleware function
 */
export const validatePhoneMiddleware = (fieldName = 'phone') => {
  return (req, res, next) => {
    ...
    if (!isValidPhone(phone)) {
      return res.status(400).json({
        success: false,
        message: getPhoneValidationError(phone),
        field: fieldName
      });
    }

    // Format phone before proceeding
    req.body[fieldName] = formatPhone(phone);
    next();
  };
};