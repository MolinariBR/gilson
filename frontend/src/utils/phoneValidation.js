/**
 * Frontend phone validation utilities for Brazilian phone numbers
...
/**
 * Validate if phone number is a landline number
 * @param {string} phone - Phone number to validate
 * @returns {boolean} True if landline, false otherwise
 */
export const isLandlinePhone = (phone) => {
  ...
  // Check formatted landline
  if (PHONE_PATTERNS.LANDLINE.test(phone)) {
    const areaCode = phone.slice(1, 3);
    return VALID_AREA_CODES.includes(areaCode);
  }

  // Check raw landline (10 digits, not starting with 9 after area code)
  const cleaned = sanitizePhone(phone);
  if (cleaned.length === 10) {
    const areaCode = cleaned.slice(0, 2);
    const firstDigit = cleaned[2];
    return VALID_AREA_CODES.includes(areaCode) && firstDigit !== '9';
  }
 ...
 * @returns {string} Error message in Portuguese
 */
export const getPhoneValidationError = (phone) => {
  if (!phone || phone.trim() === '') {
    return getTranslation('validation.phoneRequired', 'Telefone é obrigatório');
  }

  const cleaned = sanitizePhone(phone);
  
  if (cleaned.length < 10) {
    return getTranslation('validation.phoneTooShort', 'Telefone deve ter pelo menos 10 dígitos');
  }
  
  if (cleaned.length > 11) {
    return getTranslation('validation.phoneTooLong', 'Telefone deve ter no máximo 11 dígitos');
  }
  
  if (cleaned.length === 10 || cleaned.length === 11) {
    const areaCode = cleaned.slice(0, 2);
    if (!VALID_AREA_CODES.includes(areaCode)) {
      return getTranslation('validation.phoneInvalidAreaCode', 'Código de área inválido');
    }
  }
  
  if (cleaned.length === 11 && cleaned[2] !== '9') {
    return getTranslation('validation.phoneMobileFormat', 'Celular deve começar com 9 após o código de área');
  }
  
  return getTranslation('validation.phoneInvalidFormat', 'Formato de telefone inválido. Use (11) 99999-9999');
};

/**
 * Phone validation hook for React forms
 * @param {string} phone - Phone number to validate
 * @returns {Object} Validation result with isValid and error message
 */
export const usePhoneValidation = (phone) => {
  const isValid = isValidPhone(phone);
  const error = isValid ? '' : getPhoneValidationError(phone);
...
/**
 * Phone input handler for React components
 * @param {Function} setValue - State setter function
 * @returns {Function} Input change handler
 */
export const createPhoneInputHandler = (setValue) => {
  return (event) => {
...
/**
 * Get phone placeholder text
 * @returns {string} Placeholder text in Portuguese
 */
export const getPhonePlaceholder = () => {
  return getTranslation('order.phonePlaceholder', '(11) 99999-9999');
};